...
1. React to user interaction
2. Interact with the web page (DOM) <====== This lecture
3. Run requests for additional data

#### Finding stuff
```js
document.querySelector(".tile") // finds first node with class 'tile'
document.querySelectorAll(".tile") // finds ALL nodes with class 'tile'
document.querySelector("div.tile") // finds first div with class 'tile'
document.querySelector("div#beef.tile") // finds first div with class 'tile' and id 'beef'

document.getElementById("beef") // finds first element with id beef
document.getElementsByTagName("div") // finds all divs
document.getElementsByClassName("tile") // finds all elements with class 'tile'
```
...
tile.innerText // returns text of DOM node
tile.innerText = "something" // changes text of DOM node



tile.style.borderWidth = "2px" // changes inline style of DOM node
...
tile.innerHTML // returns internal HTML of node as a string
tile.innerHTML = "<p>something</p>" // overwrites old internal HTML and replaces new HTML

tile.insertAdjacentHTML("beforeend", "<p>something</p>") // appends new HTML without overwriting old HTML

const newDiv = document.createElement("div") // makes empty DOM node
newDiv.innerText = "Some divy thing" // modify new DOM node
tile.append(newDiv) // attach new DOM node to some other DOM node
```
